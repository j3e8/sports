...
const url = 'https://search.yahoo.com/search?p=%22TSHQ+License+Agreement%22+email&fr=yfp-t&fp=1&toggle=1&cop=mss&ei=UTF-8';
...
function parseSearchResults (html) {
  const match_r = /<a[^>]+href="[^"]+RU=([^"]+)\/RK=[^"]+"[^>]+referrerpolicy="origin"[^>]+>/gmi;
  const matches = [];
  let m;
  ...
  const filtered = matches.filter(m => !m.includes('yahoo') && !m.includes('gc.com') && !m.includes('yimg') && m[0] !== '/' && !m.includes('bluesombrero'));
...
function matchNextPageLink(html) {
  const next_r = /<a[^>]+href="([^"]+)"[^>]+>Next<\/a>/gmi;
...
if (parseInt(START_PAGE) !== 0) {
  initialUrl += `&b=${START_PAGE * PER_PAGE + 1}`;
}
